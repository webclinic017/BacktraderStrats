...
def unlock_trade(trade_context=trade_context, trading_pwd=TRADING_PWD, trading_env=TRADING_ENVIRONMENT):
    if trading_env == TrdEnv.REAL:
        ...
        if ret != RET_OK:
            print('unlock trade failure：', data)
            return False
        print('unlock trade successfully!')
    return True
...
def is_normal_trading_time(code):
    ...
    if ret != RET_OK:
        print('retrieve market state failure：', data)
        return False
    ...
    if market_state == MarketState.MORNING or \
        ...
        return True
    print('market not in traidng hour')
    return False
...
def get_ask_and_bid(code):
    ...
    if ret_sub == RET_OK:
        ...
        print('code: %s subscription failure, ERR Message: %s' % (code, data_sub))
...
def get_holding_position(code, trd_env=TRADING_ENVIRONMENT):
    ...
    if ret != RET_OK:
        print('retrieve holding position failure：', data)
        return None
    else:
        if data.shape[0] > 0:
            holding_position = data['qty'][0]
        print('【holding status】 {} holding positions：{}'.format(code, holding_position))
...
def open_position(code, open_quantity):
    ...
    if is_valid_quantity(code, open_quantity, ask):
        ...
        if ret != RET_OK:
            print('开仓失败：', data)
        print('code %s open position successfully! quantity: %d' % (code, open_quantity))
    else:
        print('下单数量超出最大可买数量。')
